package chap15_stringsrevisited.examples;

import org.testng.Assert;
import org.testng.annotations.Test;

public class StringBuilderTest {

    @Test
    public void canConstructStringBuilder(){
        StringBuilder builder = new StringBuilder();
        Assert.assertEquals(builder.capacity(), 16);
        Assert.assertEquals(builder.toString().length(), 0);

        // capacity space allocated in memory, not the string length
        // StringBuilder will automatically resize
        StringBuilder sblen = new StringBuilder(512);
        Assert.assertEquals(sblen.capacity(), 512);
        Assert.assertEquals(sblen.toString().length(), 0);

        StringBuilder sb = new StringBuilder("hello");
        Assert.assertEquals(sb.toString().length(), 5);
    }

    @Test
    public void quickStringBuilderDemo(){
        StringBuilder builder = new StringBuilder();
        builder.append("Hello There").
                replace(7,11,"World").
                delete(5,7);
        Assert.assertEquals(builder.toString(), "HelloWorld");
    }

    @Test
    public void appendToStringBuilder(){
        StringBuilder builder = new StringBuilder();
        builder.append("> ");
        builder.append(1);
        builder.append(" + ");
        builder.append(2);
        char[] ca = {' ', '=', ' ', '3'};
        builder.append(ca);

        Assert.assertEquals(builder.toString(), "> 1 + 2 = 3");
    }


    @Test(expectedExceptions=StringIndexOutOfBoundsException.class)
    public void insertOutsideIntoStringBuilderWhichIsEmpty(){
        StringBuilder builder = new StringBuilder();
        builder.insert(5,"Hello");
    }

    @Test
    public void insertIntoStringBuilder(){
        StringBuilder builder = new StringBuilder("123890");
        builder.insert(3,"4567");
        Assert.assertEquals(builder.toString(), "1234567890");
    }


    @Test
    public void insertCharArrayIntoStringBuilder(){
        char[] ca = {'.', 'a', 'b', 'c', 'd', 'e', 'f'};
        StringBuilder builder = new StringBuilder("abgh");
        // at position 2 in the string
        // insert from the char[] ca
        // starting at index 3 'c'
        // inclusive the next 4 indexes
        builder.insert(2, ca, 3, 4);
        Assert.assertEquals(builder.toString(), "abcdefgh");
    }

    @Test(expectedExceptions=StringIndexOutOfBoundsException.class)
    public void insertOutsideStringBuilder(){
        StringBuilder builder = new StringBuilder("a");
        builder.insert(2,"Hello");
    }

    @Test
    public void deleteFromStringBuilder(){
        StringBuilder builder = new StringBuilder("abcdefg");
        builder.delete(2,4);
        Assert.assertEquals(builder.toString(), "abefg");
    }

    @Test
    public void deleteSingleCharFromStringBuilder(){
        StringBuilder builder = new StringBuilder("abcdefg");
        builder.deleteCharAt(2);
        Assert.assertEquals(builder.toString(), "abdefg");
    }

    @Test
    public void ensureCapacityForStringBuilder(){
        StringBuilder builder = new StringBuilder("abcdefg");
        builder.ensureCapacity(600);
        Assert.assertEquals(builder.capacity(), 600);
        Assert.assertEquals(builder.toString(), "abcdefg");
    }

    @Test
    public void capacityManagementWithTrimToSize(){
        StringBuilder builder = new StringBuilder(600);
        Assert.assertEquals(builder.capacity(), 600);

        builder.append("hello");
        Assert.assertEquals(builder.capacity(), 600);
        builder.trimToSize();
        Assert.assertEquals(builder.capacity(), 5);
    }

    @Test
    public void replaceStringBuilderSubStrings(){
        StringBuilder builder = new StringBuilder("abcdefgh");
        builder.replace(0,4,"12345678");
        Assert.assertEquals(builder.toString(), "12345678efgh");
    }

    @Test
    public void replaceStringBuilderChar(){
        StringBuilder builder = new StringBuilder("012345678");
        builder.setCharAt(5,'f');
        Assert.assertEquals(builder.toString(), "01234f678");
    }

    @Test
    public void reverseStringBuilder(){
        StringBuilder builder = new StringBuilder("0123456789");
        Assert.assertEquals(builder.reverse().toString(), "9876543210");
    }

    @Test
    public void substringStringBuilder(){
        StringBuilder builder = new StringBuilder("0123456789");
        Assert.assertEquals(builder.substring(3,7), "3456");
        Assert.assertEquals(builder.substring(3), "3456789");
    }
}
